...
internal fun GamesView(
    ...
    listDisplayMode: ListDisplayOption,
    ...
    when (pagingItems.loadState.refresh) {
        ...
        is LoadState.NotLoading if pagingItems.itemCount == 0 -> {
            ...
            LazyVerticalGrid(
                ...
                columns = GridCells.Adaptive(if (listDisplayMode == ListDisplayOption.Grid) 160.dp else 300.dp),
                ...
                if (pagingItems.loadState.refresh is LoadState.Loading) {
                    items(
                        count = if (listDisplayMode == ListDisplayOption.Grid) 40 else 20,
                        key = { "LoadingStub#$it" }) {
                        LaunchedEffect(Unit) {
                            gridState.scrollToItem(0)
                        }
                        if (listDisplayMode == ListDisplayOption.Grid) {
                            GameGridItemView(
                                game = null,
                                isLoading = true,
                            )
                            ...
                            GameListItemView(
                                game = null,
                                isLoading = true,
                                isLarge = listDisplayMode == ListDisplayOption.Large,
                            )
                        }
                    }
                }
                ...
                if (pagingItems.loadState.append == LoadState.Loading) {
                    items(
                        count = if (listDisplayMode == ListDisplayOption.Grid) 20 else 10,
                        key = { "LoadingStub#$it" }) {
                        if (listDisplayMode == ListDisplayOption.Grid) {
                            GameGridItemView(
                                game = null,
...
private fun GamesViewPreview_Loading() {
    JetGamesTheme {
        ...
        GamesView(
            ...
            pagingItems = games,
            listDisplayMode = ListDisplayOption.Grid,
            onRefreshPressed = { },
            onGameSelected = {})
    }
}
