...
internal fun ScreenshotsView(
    ...
    LazyRow(
        ...
        when (screenshots) {
            is LoadingModel.Error -> item {
                Box(
                    modifier = modifier.fillMaxSize(),
                    contentAlignment = Alignment.Center
                ) {
                    Text(text = screenshots.throwable.localizedMessage ?: "Error")
                }
            }

            LoadingModel.Loading -> items(10) {
                ScreenshotItem(
                    isLoading = true,
                    imageUrl = "",
                )
            ...
            is LoadingModel.Success -> if (screenshots.value.isEmpty()) {
                item {
                    Box(
                ...
                itemsIndexed(screenshots.value) { index, item ->
                    ScreenshotItem(
                        isLoading = false,
