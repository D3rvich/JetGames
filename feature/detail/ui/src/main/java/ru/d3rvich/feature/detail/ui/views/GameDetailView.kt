...
internal fun GameDetailView(
    ...
    screenshots: LoadingModel<ImmutableList<ScreenshotEntity>>,
    ...
    Scaffold(
        ...
        topBar = {
            GameDetailAppBar(
                title = "${gameDetail.name} ${
                    gameDetail.ratings?.maxByOrNull { it.percent }?.findWrapper()?.textIcon ?: ""
                }",
        ...
        LazyColumn(
            ...
            if (gameDetail.metacritic != null || gameDetail.rating != null) {
                item {
                    RatingsView(metacritic = gameDetail.metacritic, rating = gameDetail.rating)
                }
            }
            gameDetail.ratings?.let { ratings ->
                if (ratings.isNotEmpty()) {
                    item {
                        RatingDetailView(ratings = ratings)
                    }
                }
            }
            ...
            gameDetail.parentPlatforms?.let { parentPlatforms ->
                if (parentPlatforms.isNotEmpty()) {
                    item {
...
private fun GameDetailViewPreview() {
    ...
    GameDetailView(
        gameDetail = detail,
        screenshots = LoadingModel.Success(persistentListOf()),
        stores = LoadingModel.Success(persistentListOf()),
        onFavoriteChange = {},
        onScreenshotClicked = {},
        onBackClicked = {},
        onGameStoreSelected = {}
